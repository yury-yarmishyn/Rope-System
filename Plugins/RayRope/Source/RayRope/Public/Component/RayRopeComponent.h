...
class RAYROPE_API URayRopeComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	...
	/** Uses the batch rail-coordinate solver before the legacy local move fallback. */
	UPROPERTY(EditAnywhere, Category = "Rope|Move Solver|Global")
	bool bUseGlobalMoveSolver = true;

	/** Runs the legacy local pass when the global pass is not applicable or cannot find a safe step. */
	UPROPERTY(EditAnywhere, Category = "Rope|Move Solver|Global")
	...
	/** Maximum number of shared-alpha backtracking attempts for each global batch step. */
	UPROPERTY(EditAnywhere, Category = "Rope|Move Solver|Global", meta = (ClampMin = "0", UIMin = "0"))
